...
public class InfiniteScroll : MonoBehaviour
{
    ...
    [SerializeField] private float minScrollTime = 4f;
    [SerializeField] private float maxScrollTime = 6f;
    [SerializeField] private float slerpDuration = 1.0f;

    [Header("UI Components")]
    public TextMeshProUGUI Bonus_Count_txt;
    public TextMeshProUGUI BonusMultiplier_txt;
    [SerializeField] private RectTransform numbCount_1, numbCount_2, numbCount_3;
    [SerializeField] private Image GlowEffect;

    public MasterAudioController audioController;

    private void OnEnable() => autoScroll = true;
    ...
    private void HandleScrollingBoundaries()
    {
        if (isUpdated)
        {
            isUpdated = false;
        ...
        if (contentPanelTransform.localPosition.x > 0)
        {
            AdjustScrollPosition(-ItemList.Length * itemWidth);
        }
        else if (contentPanelTransform.localPosition.x < -(ItemList.Length * itemWidth))
        {
            AdjustScrollPosition(ItemList.Length * itemWidth);
        }
    }

    private void AdjustScrollPosition(float adjustment)
    {
        Canvas.ForceUpdateCanvases();
        oldVelocity = scrollRect.velocity;
    ...
    private void ShuffleContentChildren()
    {
        ...
        {
            int randomIndex = Random.Range(0, contentPanelTransform.childCount);
            contentPanelTransform.GetChild(i).SetSiblingIndex(randomIndex);
        }
    }
    ...
    private IEnumerator AutoScrollForRandomTime()
    {
        ...
        for (float elapsed = 0; elapsed < scrollDuration; elapsed += Time.deltaTime)
        {
            float direction = Count == 2 ? -1 : 1;
            contentPanelTransform.localPosition += new Vector3(direction * scrollSpeed * Time.deltaTime, 0, 0);
            yield return null;
        }
    ...
    private IEnumerator CenterOnClosestItem()
    {
        ...
        foreach (RectTransform item in contentPanelTransform)
        {
            float distance = Mathf.Abs(viewPortTransform.InverseTransformPoint(item.position).x -
                                       viewPortTransform.InverseTransformPoint(targetPositionObject.transform.position).x);
        ...
        if (closestItem != null)
        {
            Vector3 adjustment = viewPortTransform.InverseTransformPoint(closestItem.position).x -
                                 viewPortTransform.InverseTransformPoint(targetPositionObject.transform.position).x;

            yield return SlerpToPosition(adjustment);

            UpdateBonusValue(closestItem);
        }
    }

    private IEnumerator SlerpToPosition(float adjustment)
    {
        Vector3 startPos = contentPanelTransform.localPosition;
        Vector3 endPos = contentPanelTransform.localPosition - new Vector3(adjustment, 0, 0);

        for (float elapsed = 0; elapsed < slerpDuration; elapsed += Time.deltaTime)
        {
            contentPanelTransform.localPosition = Vector3.Slerp(startPos, endPos, elapsed / slerpDuration);
            yield return null;
        }

        contentPanelTransform.localPosition = endPos;
    }

    private void UpdateBonusValue(RectTransform closestItem)
    {
        TextMeshProUGUI textComponent = closestItem.GetComponentInChildren<TextMeshProUGUI>();
        if (textComponent != null)
        {
            controller.BonusRewardValue = textComponent.text;
            if (float.TryParse(controller.BonusRewardValue.TrimEnd('x'), out bfloat))
            {
                BonusValue += bfloat;
            }

            controller.BonusRewardTxt.text = textComponent.text;
            controller.BonusRewardTxt.color = Color.green;
    ...
    private async void DelayFuction()
    {
        if (Count == 1)
        {
    ...
    private async void BounsAmount_Moves()
    {
        Count--;
        Sequence sequence = DOTween.Sequence();

        sequence.Append(controller.BonusRewardTxt.transform.DOScale(Vector3.one * 3f, 0.5f).SetEase(Ease.InOutSine))
                .Join(controller.BonusRewardTxt.DOColor(new Color32(110, 0, 0, 255), 0.01f).SetEase(Ease.Linear))
                .AppendInterval(1f);

        Invoke(nameof(Object_Delay), 1.6f);

        sequence.Append(controller.BonusRewardTxt.transform.DOScale(Vector3.one, 0.5f).SetEase(Ease.InOutSine))
                .Join(controller.BonusRewardTxt.DOColor(new Color32(95, 255, 0, 255), 0.01f).SetEase(Ease.Linear));

        await UniTask.Delay(200);
        controller.BonusRewardTxt.text = $"+{controller.BonusRewardTxt.text}";
    }
    ...
    private void Object_Delay()
    {
        StartCoroutine(Add_BonusValue());
        controller.BonusRewardTxt.text = null;
    }

    private IEnumerator Add_BonusValue()
    {
        while (controller.Multiplier < BonusValue)
        {
            controller.Multiplier = Mathf.Min(controller.Multiplier + bfloat * Time.deltaTime, BonusValue);
            BonusMultiplier_txt.text = controller.Multiplier.ToString("0.00");
            yield return null;
        }
    }

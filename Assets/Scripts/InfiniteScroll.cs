...
public class InfiniteScroll : MonoBehaviour
{
    ...
    [SerializeField] GameObject targetPositionObject; // Target position GameObject
    ...
    [SerializeField] TextMeshProUGUI bonus_Count_txt;
    ...
    void CountText_Animation()
    {
        ...
        // Determine the current and previous elements based on the count
        switch (Count)
        {
            case 1:
                current = numbCount_1;
                previous = numbCount_2;
                break;
            case 2:
                current = numbCount_2;
                previous = numbCount_3;
                break;
            case 3:
                current = numbCount_3;
                previous = numbCount_1;
                break;
        }

        if (current != null && previous != null)
        {
            // First animation: Hide previous, scale up current, and apply glow
            sequence.Append(previous.DOScale(Vector3.zero, 0.01f).SetEase(Ease.InOutSine))
                    .Join(current.DOScale(new Vector3(1.2f, 1.2f, 1.2f), 0.5f).SetEase(Ease.InOutSine))
                    .Join(GlowEffect.DOColor(new Color32(255, 255, 255, 255), 1f).SetEase(Ease.Linear));

            // Delay
            sequence.AppendInterval(0.01f);

            // Second animation: Scale current to normal and fade out glow
            sequence.Append(current.DOScale(Vector3.one, 0.5f).SetEase(Ease.InOutSine))
                    .Join(GlowEffect.DOColor(new Color32(255, 255, 255, 0), 1f).SetEase(Ease.Linear));
        }
    }

    #endregion :::::::::::: CountText_Animation NEW ::::::::::::

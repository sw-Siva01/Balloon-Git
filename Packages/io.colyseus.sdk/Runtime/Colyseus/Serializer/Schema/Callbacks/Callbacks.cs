		...
		protected Action AddCallbackOrWaitCollectionAvailableSimple(Schema instance, string propertyName, OPERATION operation, Delegate handler, bool immediate = true)
		{
			...
			else
			{
				...
				if (operation == OPERATION.ADD && immediate)
				{
					...
					{
						try { handler.DynamicInvoke(key, value); }
						catch (Exception e) { ColyseusContext.Logger.LogError(e.Message); }
					});
				}
					...
					if (!UniqueRefIds.Contains(refId))
					{
						...
						if (replaceCallbacks != null)
						{
							foreach (var callback in replaceCallbacks)
							{
								try
								{
									// Guard: schema REPLACE callbacks must be 0-param. A 2-param callback
									// here means it was mis-registered on a schema refId (reconnect before
									// liveBets receives its server-assigned refId from the server).
				...
				else
				{
					...
					if ((change.Op & (byte)OPERATION.DELETE) == (byte)OPERATION.DELETE)
					{
						if (change.PreviousValue != container.GetTypeDefaultValue())
						{
							...
							if (deleteCallbacks != null)
							{
								...
								{
									try { callback.DynamicInvoke(change.DynamicIndex ?? change.Field, change.PreviousValue); }
									catch (Exception e) { ColyseusContext.Logger.LogError(e.Message); }
								}
							}
						}
						...
						if ((change.Op & (byte)OPERATION.ADD) == (byte)OPERATION.ADD)
						{
							...
							if (addCallbacks != null)
							{
								...
								{
									try { callback.DynamicInvoke(change.DynamicIndex, change.Value); }
									catch (Exception e) { ColyseusContext.Logger.LogError(e.Message); }
								}
								isTriggering = false;
							}
					...
					)
					{
						...
						if (addCallbacks != null)
						{
							...
							foreach (var callback in addCallbacks)
							{
								try { callback.DynamicInvoke(change.DynamicIndex ?? change.Field, change.Value); }
								catch (Exception e) { ColyseusContext.Logger.LogError(e.Message); }
							}
							isTriggering = false;
						}
